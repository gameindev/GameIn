...
const FavoriteUserAvatar = ({ favoriteUser, onRemove }) => {
    ...
    const displayName =
        profile?.first_name || profile?.last_name
            ...
            : profile?.brand_name || favoriteUser?.name || favoriteUser?.username || "";
                                ...
                                {hasChildren ? (
                                    ...
                                    >
                                        <Accordion.Item value={item.label.toLowerCase()}>
                                            <Accordion.Control px={"1.5em"} icon={item.icon} className={active ? "active" : ""}>
                                                {item.label}
                                            </Accordion.Control>
                                                ...
                                                <ul>
                                                    {item.children.map((child, cIndex) => (
                                                        <li key={cIndex}>
                                                            <Link
                                                                to={child.link}
                                                                className={pathname.startsWith(child.link) ? "active" : ""}
                                                            >
                                                                {child.icon}
                                    ...
                                    <Link to={item.defaultLink || item.link} className={active ? "active" : ""}>
                                        {item.icon}
