import { updateBrandProfileService } from "../service/updateBrandProfile.service";
...
const useUpdateBrandProfile = (setError, dispatch, setUserAction, currentUser) => {
    const updateBrandProfile = useCallback(async (userId, data) => {
        setError("");
        
        try {
            const response = await updateBrandProfileService(userId, data);
            
            showNotificationHelper("Success", "Brand profile updated successfully", NOTIFICATION_TYPES.SUCCESS);
            setError("");
