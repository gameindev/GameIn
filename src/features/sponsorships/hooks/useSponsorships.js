...
import {
  ...
  negotiateOffering,
  resetOffering,
} from "../services/fetchSponsorships";
...
export default function useSponsorships({
    ...
    async function load() {
      try {
        ...
        const data = await fetchSponsorships({
          get,
          user: { id: userId, user_type: profileUserType },
          page,
          limit,
        });
  ...
  }, [userId, profileUserType, page, limit, get]);
  ...
  const handleResetOffering = async (offeringId, options = {}) => {
    ...
    try {
      ...
      console.error("Error reset offering:", err);
      throw err;
    }
  };
