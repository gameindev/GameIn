import { ActionIcon, Box, Button, Flex, Grid, Group, Image, memoize, Text, Textarea } from "@mantine/core";
...
const EditOpportunity = ({ rolePermissions }) => {
    ...
    const { get } = useApi();
    const { initiatePayment, verifyPayment, loading: paymentLoading } = usePayment();
    ...
    const handleProceedPayment = async (data) => {
        ...
        if (data?.action === "verify" && data?.paymentIntentId && data?.providerPaymentId) {
            try {
        ...
        try {
            if (!offerings?.data?.id) {
                showNotificationHelper("Error", "Offering data not available", NOTIFICATION_TYPES.ERROR);
                return;
            }
            ...
            if (
                (paymentProvider === PaymentProvider.STRIPE && paymentResponse?.clientSecret) ||
                (paymentProvider === PaymentProvider.RAZORPAY && paymentResponse?.razorpayOrderId)
            ) {
    ...
    const { control, handleSubmit, watch, setValue, reset } = useOfferingFormHandler({
        defaultValues: editDefaultValues,
        onSubmit: (data) => {
            if (brandLocked) {
                showNotificationHelper("Edit Locked", "You cannot edit this sponsorship until the timer expires.", NOTIFICATION_TYPES.WARNING);
                return;
            }
            ...
            submitEdit(
                ...
                () => {
                    showNotificationHelper("Sponsorship Updated", "Your sponsorship has been updated successfully.", NOTIFICATION_TYPES.SUCCESS);
                    navigate(-1);
                }
            ).catch((err) => {
                showNotificationHelper("Error", err?.response?.data?.message || err?.message || "Something went wrong", NOTIFICATION_TYPES.ERROR);
                console.error("Error updating sponsorship:", err);
            });
        },
    });

    const { isCreator, brandLocked, timerActive, lastEditedByType, isAccepted } = useEditLock(
        {
            user,
    ...
    useEffect(() => {
        if (offerings.data) {
            ...
            if (offerings.data.is_terms_signed !== undefined) {
                formValues.terms = {
                    ...formValues.terms,
                    acknowledgement: Boolean(offerings.data.is_terms_signed),
                };
            }
            reset(formValues);
        }
    ...
    return (
        <FormDisableProvider disabled={brandLocked || (!sponsorEdit && !isCreator)}>
            <Box component="form" onSubmit={handleSubmit}>
                ...
                <Grid gutter={20}>
                        ...
                        {!isCreator && (
                            <Flex gap={20} align="center" mt="lg">
                                <ActionIcon size="lg" color="inputBgColor" variant="filled"
                                        onClick={() => routeService.messageRoute(
                    ...
                    <UploadLogoField control={control} offering={offerings.data} />
                ...
                <Box w="100%" h={1} style={{ borderBottom: "1px dashed #50565a" }} mt={20} />
