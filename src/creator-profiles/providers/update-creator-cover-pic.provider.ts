...
export class UpdateCreatorCoverPicProvider {
    ...
    public async updateCreatorCoverPic(profileId: number, coverImageFile: Express.Multer.File) { 
        ...
        if (coverImageFile) {
            if (creatorProfile.coverImage) {
                // Step 1: Unlink FK
                const oldUpload = creatorProfile.coverImage;
                creatorProfile.coverImage = null;
            ...
            creatorProfile.coverImage = await this.uploadService.uploadNew(coverImageFile);
        }
